...
#[derive(Serialize, Deserialize)]
pub struct ClubRow { pub id: i64, pub name: String, pub short_name: String, pub nation: String, pub nation_id: i64, pub city: String, pub city_id: Option<i64>, pub stadium: String, pub capacity: i64, pub reputation: i64, pub primary_color: String, pub secondary_color: String }
#[derive(Serialize, Deserialize)]
...
pub async fn list_clubs(pool: &SqlitePool) -> Result<Vec<ClubRow>, String> {
    let rows = sqlx::query_as::<_, (i64, String, String, String, i64, Option<String>, Option<i64>, Option<String>, Option<i64>, i64, String, String)>(
        "SELECT c.id, c.name, c.short_name, n.name, c.nation_id, ci.name, c.city_id, s.name, s.capacity, c.reputation, c.primary_color, c.secondary_color FROM clubs c JOIN nations n ON n.id=c.nation_id LEFT JOIN cities ci ON ci.id=c.city_id LEFT JOIN stadiums s ON s.id=c.stadium_id ORDER BY c.reputation DESC, c.name"
    ).fetch_all(pool).await.map_err(|e| e.to_string())?;
    Ok(rows.into_iter().map(|(id, name, short_name, nation, nation_id, city, city_id, stadium, capacity, reputation, primary_color, secondary_color)| ClubRow { id, name, short_name, nation, nation_id, city: city.unwrap_or_default(), city_id, stadium: stadium.unwrap_or_default(), capacity: capacity.unwrap_or(2000), reputation, primary_color, secondary_color }).collect())
}
pub async fn list_players(pool: &SqlitePool, limit: i64) -> Result<Vec<PlayerRow>, String> {
    let lim = limit.clamp(20, 500);
