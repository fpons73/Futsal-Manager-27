...
pub async fn rollover_season(pool: &SqlitePool) -> Result<String, String> {
    ...
    // Clear old standings and generate new
    // Keep history, insert new season standings
    let comps2: Vec<(i64,)> = sqlx::query_as("SELECT id FROM competitions").fetch_all(pool).await.map_err(|e| e.to_string())?;
    for (cid,) in comps2 {
        // Get clubs for this competition (same set)
        let clubs: Vec<(i64,)> = sqlx::query_as("SELECT club_id FROM league_standings WHERE competition_id=? AND season=?").bind(cid).bind(&season).fetch_all(pool).await.map_err(|e| e.to_string())?;
        for (club_id,) in clubs {
            sqlx::query("INSERT INTO league_standings(competition_id, season, club_id, position, played, won, drawn, lost, goals_for, goals_against, goal_difference, points) VALUES(?,?,?,?,0,0,0,0,0,0,0,0)")
                .bind(cid).bind(&next_season).bind(club_id).bind(0).execute(pool).await.map_err(|e| e.to_string())?;
        }
        // Update competitions season
    ...
    let msg = format!("Temporada {} finalizada. {} jugadores retirados. Nueva temporada {} comienza el {}.", season, retired, next_season, new_date.format("%Y-%m-%d"));
